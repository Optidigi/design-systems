...
export const FloatingToolbar: React.FC<{
  ...
  const Inner = (
    <div role="toolbar" className={cn("rt-floating-toolbar flex items-center gap-0.5", FLOATING_PILL_CLASS)}>
      ...
      {/* ColorChip is reserved for the RichText block (allowColor flag
  ...
  if (isMobile) {
    return createPortal(
      <div
        className="fixed inset-x-2 z-[9999] overflow-x-auto"
        style={{ bottom: `calc(${kbOffset}px + env(safe-area-inset-bottom, 0px) + 8px)` }}
